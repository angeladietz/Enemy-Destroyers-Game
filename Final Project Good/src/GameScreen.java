...
public class GameScreen {
	...
	private void initialize() {
		...
		Gframe.setContentPane(new JLabel(new ImageIcon("space.png")));
		...
		public void actionPerformed(ActionEvent e) {//actions done on timer for enemy
			...
			for (int i = 0; i<enemies.size(); i++){
				if (u.getLabel().getY()<enemies.get(i).getLabel().getY()+50){
					//if (intersects(u.getLabel(), enemies.get(i).getLabel())==false){
			...
			for (int i=0; i<bShooter.size(); i++){
				for (int j=0;j<enemies.size(); j++){
					if (intersects(bullets.get(i), enemies.get(j).getLabel())){
						...
						bullets.get(i).setLocation(-100,-100);
	...
	private class actTimer2 implements ActionListener{
		public void actionPerformed(ActionEvent e) {//actions done on timer for enemy
			...
			int rC = (int)Math.floor((Math.random()*(enemies.size()))); //generates a random number to move a random enemy down

			enemies.get(rC).Attack(); //gets the random enemy and moves it down
		}
	}
